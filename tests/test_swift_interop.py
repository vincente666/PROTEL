...
def test_protel_calls_swift_emits_external_swift_greet():
    source = (INTEROP / "protel_calls_swift.P").read_text(encoding="utf-8")
    cpp = transpile_to_cpp(parse_protel(source), source_name="protel_calls_swift.P")
...
def test_protel_for_swift_exports_c_linkage():
    source = (INTEROP / "protel_for_swift.P").read_text(encoding="utf-8")
    cpp = transpile_to_cpp(parse_protel(source), source_name="protel_for_swift.P")
