...
def _transpile_hello() -> str:
    ...
    return transpile_to_cpp(tree, source_name="Hello.protel")
...
def test_cpp_hello_emits_namespace_and_main():
    ...
    assert "IS ENTRY" in (EXAMPLES / "Hello.protel").read_text(encoding="utf-8")
    assert "void Start()" in cpp
    assert 'extern "C"' in cpp
    assert "void writeln(const char* msg)" in cpp
    assert "<iostream>" not in cpp
    assert "IS INTRINSIC" not in (EXAMPLES / "Hello.protel").read_text(encoding="utf-8")
    assert "EXTERNAL" in (EXAMPLES / "Hello.protel").read_text(encoding="utf-8")
...
def test_cpp_hello_class_uses_this():
    source = (EXAMPLES / "ref_oo_class_with_var.protel").read_text(encoding="utf-8")
    cpp = transpile_to_cpp(parse_protel(source), source_name="ref_oo_class_with_var.protel")
...
def test_hello_shebang_direct_execution():
    hello = EXAMPLES / "Hello.protel"
    assert hello.read_text(encoding="utf-8").startswith("#!/usr/bin/env protel-run")
...
def test_hello_compiles_and_runs_a_out():
    result = subprocess.run(
        [str(ROOT / ".venv/bin/python"), str(PROTEL), "examples/Hello.protel", "--run"],
        capture_output=True,
        text=True,
        cwd=ROOT,
    )
...
def test_cpp_oo_examples_compile():
    ...
    for name in ("ref_oo_class_with_var.protel", "ref_oo_class_method_invoke.protel"):
        source = EXAMPLES / name
        tree = parse_protel(source.read_text(encoding="utf-8"))
        cpp = transpile_to_cpp(tree, source_name=name)
        cpp_path = ROOT / "build" / f"{name.replace('.protel', '.cpp')}"
